...
//                            "VALUES('" + userID + "', 1, 'FARM LAB01', 'NOTA NA', 'SVILUPPO', SYSDATE)");
    ...
    void insertQUser(String userID,
        ...
        try {
            if (getConnection() == null) {
                ...
                if (resultSet.getInt(1) == 0) {
                    loggingMisc.printConsole(1, "Executing query");
                    loggingMisc.printConsole(1, "INSERT INTO WADA.Q_USERS (ID, USERID, NAME)" +
                            " VALUES(WADA.Q_USERS_SEQ.NEXTVAL, '" + userID + "', '" + name + "');");
                    resultSet = statement.executeQuery("INSERT INTO WADA.Q_USERS (USERID, NAME)" +
                            "VALUES('" + userID + "', '" + name + "')");
                    loggingMisc.printConsole(1, "Executing query - successful");
                } else {
    ...
    public void insertQUserAttribute() {
    ...
    void insertUsers(String userID,
        ...
        try {
            if (getConnection() == null) {
                ...
                statement = getConnection().createStatement();
                loggingMisc.printConsole(1, "Creating statement Successful");
                statement.executeQuery("select count(*) FROM WADA.USERS_1 WHERE USERID LIKE '" + userID + "';");
                resultSet.next();
                if (resultSet.getInt(1) == 0) {
                    loggingMisc.printConsole(1, "Executing query");
